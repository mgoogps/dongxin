...
(function($, owner) {
	...
	owner.login = function(loginInfo, callback) {
		...
		var data = {
		 	...
		 	identifies: "MgooApp@BAIDU",
		 	code:''
		}; 
		var url = "http://106.14.24.147/ajax/Login.asmx/MgLogin";
		var authed = false; 
		mui.ajax(url,{
			...
			error:function (XMLHttpRequest, type, errorThrown) {
				 console.log( "登录出错..")	
			     console.log(JSON.parse(XMLHttpRequest))	
			}
		});  
	};
	...
	owner.setCurrentDeviceID = function(DeviceID){
		localStorage.setItem("CurrentDeviceID", DeviceID); 
	}
	owner.getCurrentDeviceID = function  () {
